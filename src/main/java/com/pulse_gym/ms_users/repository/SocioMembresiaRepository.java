...
public interface SocioMembresiaRepository extends JpaRepository<SocioMembresia, Long> {
    ...
    @Query(value = "SELECT DISTINCT sm.* FROM socio_membresia sm " +
            ...
            "WHERE sm.estado = 'ACTIVA' " +
            "AND (:busqueda IS NULL OR CONCAT(s.nombre, ' ', s.apellido) ILIKE CONCAT('%', :busqueda, '%') "
            +
    ...
    /**
     * Busca socios activos de una membresía específica con búsqueda por nombre
     * completo o email
     * 
     * @param idMembresia ID de la membresía
     * @param busqueda    Búsqueda por nombre completo o email
     * @param pageable    Configuración de paginación
     * @return Página de socios activos de la membresía
     */
    ...
    /**
     * Busca todas las membresías asignadas a un socio específico
     * 
     * @param idUsuario ID del socio
     * @return Lista de membresías asignadas al socio
     */
    List<SocioMembresia> findBySocio_IdUsuario(Long idUsuario);
}